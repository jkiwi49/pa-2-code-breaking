...
**NAMES:** Ronit Kumar and Kiwi Cervantes
```{r}
#| label: Setup
#| echo: False #Hiding code.
# We are going to load tidyverse in this setup chunk.
library(tidyverse)
...
my_dataframe <- data.frame(my_symbols) # Making dataframe from my_symbol.
colnames(my_dataframe)[1] <- "symbol" #Changing column name.
count(my_dataframe) #Total number of symbols in our dataframe.
singular_df <- mutate(my_dataframe,symbol_id = as.numeric(factor(my_symbols))) # Adds symbol id to each symbol
...
```{r}
#| label: add-label-2
step1<-top_secret+14 #add
step2<-step1*18
step3<-step2-257 #mult and sub
step4<-exp(step3) #exp
step5<-step4^2 #square
```
...
```{r}
#| label: add-label-4
 vector_1 <- matrix(data = step5 , ncol = 5)
